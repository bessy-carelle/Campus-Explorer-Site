...
try {
    ...
    $idUser = isset($_GET['idUser']) ? $_GET['idPhoto'] : null;

    // Préparation de la requête SQL
    if (!empty($idPhoto)) {
        $stmt = $con->prepare("SELECT idPhoto,idUser, nomPhoto, data, idCible FROM Photos WHERE idPhoto = :idPhoto");
        $stmt->bindParam(':idPhoto', $idPhoto, PDO::PARAM_INT);
    } else if(!empty($idUser)){
        $stmt = $con->prepare("SELECT idPhoto,idUser, nomPhoto, data, idCible FROM Photos WHERE idUser = :idUser");
        $stmt->bindParam(':idUser', $idUser, PDO::PARAM_INT);
    }else {
        $stmt = $con->prepare("SELECT idPhoto,idUser, nomPhoto, data, idCible FROM Photos");
    }
